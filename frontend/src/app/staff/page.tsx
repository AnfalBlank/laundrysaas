...
import { Plus, Mail, Phone, Crown, BriefcaseBusiness, Sparkles, Bike } from "lucide-react";
import { listStaff } from "@/db/repositories";
...
export default async function StaffPage() {
  ...
  for (const s of staffs) counts[s.role] = (counts[s.role] ?? 0) + 1;
    ...
    <AppShell title="Staff Management" subtitle="Kelola tim dan akses karyawan">
      ...
      <Card className="mt-4 sm:mt-5 overflow-hidden">
        <CardHeader className="flex items-center justify-between flex-row gap-3 space-y-0">
          <CardTitle>Daftar Karyawan ({staffs.length})</CardTitle>
